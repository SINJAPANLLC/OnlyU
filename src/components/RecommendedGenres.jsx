...
const RecommendedGenres = ({ likedItems, toggleLike }) => {
    ...
    return (
        ...
        >
            ...
            {/* Responsive 2-column grid for all screen sizes */}
            <div className="grid grid-cols-2 sm:grid-cols-2 gap-3">
                {genreData.map((genre, index) => (
                    <motion.div
                        ...
                        // onClick={() => toggleLike(genre.name)}
                        onClick={() => handleGenreClick(genre.name)}
                        className="bg-white border border-gray-200 rounded-lg p-4 flex items-center justify-between cursor-pointer shadow-sm hover:shadow-md transition-all group"
                    >
                        <div>
                            <h3 className="font-semibold text-pink-600 group-hover:text-pink-700 text-sm sm:text-base">
                                {t(`genres.${genre.nameKey}`)}
                            </h3>
                            <p className="text-xs text-gray-500">{t("genres.posts", { count: genre.count.toLocaleString() })}</p>
                        </div>
