    ...
    <div className="relative w-full h-screen bg-black overflow-hidden">
      ...
      <div className="w-full max-w-sm mx-auto h-full relative sm:max-w-md md:max-w-lg lg:max-w-xl">
        ...
        <motion.div
          ...
          className="absolute right-3 bottom-20 flex flex-col space-y-6 z-20"
        >
          ...
          >
            <div className="w-14 h-14 bg-gray-400 rounded-full border-2 border-white overflow-hidden relative">
              {post.userAvatar ? (
                <img 
                  src={post.userAvatar} 
                  alt={post.userName}
          ...
          >
            <motion.div
              animate={{ scale: likedPosts.has(post.id) ? 1.3 : 1 }}
              transition={{ type: "spring", stiffness: 500 }}
              className="w-12 h-12 bg-black bg-opacity-30 rounded-full flex items-center justify-center backdrop-blur-sm"
            >
              <Heart
                size={24}
                className={`${likedPosts.has(post.id) ? 'text-red-500 fill-red-500' : 'text-white'} transition-colors`}
              />
            </motion.div>
          ...
          <motion.div
            ...
            className="flex flex-col items-center space-y-1"
          >
            <div className="w-12 h-12 bg-black bg-opacity-30 rounded-full flex items-center justify-center backdrop-blur-sm">
          ...
          >
            <div className="w-12 h-12 bg-black bg-opacity-30 rounded-full flex items-center justify-center backdrop-blur-sm">
              <Bookmark
                size={24}
                className={`${bookmarkedPosts.has(post.id) ? 'text-yellow-400 fill-yellow-400' : 'text-white'} transition-colors`}
              />
            </div>
          </motion.div>
          ...
          >
            <div className="w-12 h-12 bg-black bg-opacity-30 rounded-full flex items-center justify-center backdrop-blur-sm">
              ...
              {/* Bottom Content for each post - TikTok Style */}
              <motion.div
                initial={{ opacity: 0, y: 50 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.4 }}
                className="absolute bottom-0 left-0 right-0 p-4 pb-20 z-20"
              >
                ...
                <div className="mb-3">
                  <div className="flex items-center space-x-2 mb-2">
                    ...
                    <motion.button
                      whileTap={{ scale: 0.95 }}
                      className="px-4 py-1 bg-red-500 text-white text-sm font-semibold rounded-full"
                    >
                      フォロー
                    </motion.button>
                  </div>
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  ...
                  {post.genres && post.genres.map((genre, genreIndex) => (
                    <motion.span
                      ...
                      className="px-3 py-1 bg-black bg-opacity-50 rounded-full text-white font-medium text-xs backdrop-blur-sm border border-white border-opacity-20"
                    >
                      #{genre}
                    </motion.span>
                  ))}
        ...
        {/* Fixed Top Bar - TikTok Style */}
        <motion.div
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
          className="absolute top-0 left-0 right-0 flex justify-between items-center p-4 z-30"
        >
          ...
          <div className="flex items-center space-x-4">
            <motion.div
              whileTap={{ scale: 0.9 }}
              className="w-8 h-8 bg-black bg-opacity-30 rounded-full flex items-center justify-center backdrop-blur-sm"
            >
