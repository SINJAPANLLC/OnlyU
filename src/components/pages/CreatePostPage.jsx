...
const CreatePostPage = () => {
    ...
    const uploadFilesToCloudinary = async (files, postId) => {
        console.log(`Starting Cloudinary upload of ${files.length} files for post ${postId}`);
        setCurrentStep(`Uploading ${files.length} file(s) to Cloudinary...`);
        setFilesUploaded(0);
        ...
        for (let index = 0; index < files.length; index++) {
            ...
            try {
                setCurrentStep(`Uploading ${file.name} to Cloudinary (${index + 1}/${files.length})`);
                ...
                console.log(`File ${index + 1} uploaded successfully to Cloudinary`);
        ...
        setCurrentStep('Files uploaded to Cloudinary successfully!');
    ...
    const handleSubmit = async () => {
        ...
        if (currentUser.isAnonymous) {
            alert('Anonymous users cannot create posts. Please sign in with an account.');
            navigate('/login');
            return;
        }
        ...
        if (!agreements.copyright || !agreements.minors || !agreements.censored) {
            alert('Please confirm all agreements before publishing');
            return;
        }

        setIsSubmitting(true);
        setUploadProgress(0);
        setCurrentStep('Creating post...');
        setFilesUploaded(0);

        try {
            console.log('Creating basic post without files first...');
            setCurrentStep('Saving post details...');
            ...
            console.log('✅ Basic post saved with ID:', docRef.id);
            setCurrentStep('Post created successfully!');

            // Only attempt file upload if files are selected
            if (uploadedFiles.length > 0) {
                console.log(`📁 Attempting to upload ${uploadedFiles.length} files...`);
                setCurrentStep('Preparing file uploads...');
                ...
                try {
                    ...
                    setCurrentStep('Finalizing post...');
                    await updateDoc(doc(db, 'posts', docRef.id), {
                        files: uploadedFileUrls
                    });
                    console.log('✅ Post updated with file URLs');
                    setCurrentStep('Post published successfully!');

                } catch (fileError) {
                    console.error('❌ File upload failed:', fileError);
                    // Don't fail the entire post - it's already saved
                    alert(`Post created successfully, but file upload failed: ${fileError.message}`);
                }
            } else {
                ...
                switch (error.code) {
                    case 'permission-denied':
                        errorMessage = 'Permission denied. Please ensure:\n' +
                                     '1. You are logged in\n' +
                                     '2. Firestore security rules allow post creation\n' +
                                     '3. Your authentication token is valid';
                        console.error('🔒 Permission denied. User:', currentUser?.uid);
                        console.error('📋 Please update Firestore security rules to allow authenticated writes');
                        break;
                    case 'unavailable':
        ...
        >
            ...
            <div className="mt-6">
                <button
                    ...
                    disabled={!agreements.copyright || !agreements.minors || !agreements.censored || isSubmitting}
                    className={`w-full font-bold py-3 rounded relative overflow-hidden ${(!agreements.copyright || !agreements.minors || !agreements.censored || isSubmitting)
